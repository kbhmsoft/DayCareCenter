...
class Common_model extends CI_Model {
   ...
   public function get_dc_data($id) {
      $this->db->select('*');
  ...
  /* public function get_three_dc_data() {
      $where_in = array('1','2','11');
      $this->db->select('*');
      $this->db->from('day_cares');
      $this->db->where_in('id',$where_in);
   ...
   public function get_user_details($id = NULL) {
    ...
    public function get_three_daycares(){
        $where_in = array('1','2','11');
        $data[''] = '-শিশু দিবাযত্ন কেন্দ্র নির্বাচন করুন-';
        $this->db->select("id, title");
        $this->db->where_in('id',$where_in);
        $this->db->from('day_cares');      
        $query = $this->db->get();
    ...
    public function get_doctors(){
      $this->db->select('*');        
      $query =  $this->db->get('demo_daycare.doctors');
