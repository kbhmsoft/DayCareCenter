   ...
   public function login(){   
      ...
      public function my_profile( $edit = 'no'){   
         ...
         if($this->input->post('hide_app_info') == '33333'){
            // Application Form
            $this->form_validation->set_rules('registrations[child_mother_name]', 'Day Cares Id', 'required|trim');
            
            

            //Validate and input data
            if ($this->form_validation->run() == true) {
            // print_r($this->input->post('hide_app_info')); exit('idfmdad');
            ...
            // Dynamic DB Name
             ...
             /*  $members_data  = $_POST['members'];
              ...
              /* if($_FILES['userfile1']['size'] > 0){
               ...
               // Send Message
               // $dc_name = $this->Site_model->get_daycare_name($this->input->post('day_cares_id'));
               $dc_name = $this->Site_model->get_daycare_name(1);
               $exp = explode(" ", $dc_name);
               $daycare_name = $exp[0];
               $user = $this->ion_auth->user()->row();
               // print_r($user);exit('user');
               // $name = $this->input->post('first_name').' '.$this->input->post('last_name');
               // $mobile = '+88'.$user->phone;
