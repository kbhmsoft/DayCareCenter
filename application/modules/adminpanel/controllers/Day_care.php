...
class Day_care extends Backend_Controller {
   ...
   public function index(){
      ...
      $this->data['results'] = $this->Common_model->get_three_dc_data('day_cares'); 
   ...
   public function details($dcID){
      if(!$this->ion_auth->is_admin()){     
         redirect('index.php/adminpanel/dashboard');
      }

      $this->data['results'] = $this->Common_model->get_dc_data($dcID); 
      

      //Load page
      $this->data['meta_title'] = 'Day Care Details';
      $this->data['subview'] = 'day_care/details';
      $this->load->view('backend/_layout_main', $this->data);
      // redirect('adminpanel/news/all');
   }
   ...
   public function edit($id){
      ...
      if ($this->form_validation->run() == true){
         ...
         if($this->Common_model->edit('day_cares', $id, 'id', $form_data)){
            ...
            if($this->ion_auth->is_admin()){
               redirect('index.php/adminpanel/day_care');
            }else if($this->ion_auth->in_group(array('dc_admin'))){
               redirect('index.php/adminpanel/day_care/my_day_care_center');
            }
         }
      }
   ...
   function delete($id) {
      ...
      redirect('index.php/adminpanel/notices/all');
   }
